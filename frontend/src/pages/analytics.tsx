import { useGetModelMetrics, getGetModelMetricsQueryKey } from "@/lib/api";
    ...
    <Card className="rounded-2xl shadow-sm border-border/60 card-hover overflow-hidden">
      <CardContent className="p-6 flex flex-col items-center gap-2">
        <div className="relative" style={{ width: 140, height: 120 }}>
          <RadialBarChart width={140} height={120} cx={70} cy={80} innerRadius={50} outerRadius={68} startAngle={180} endAngle={0} data={data}>
            <PolarAngleAxis type="number" domain={[0, 100]} angleAxisId={0} tick={false} />
            <RadialBar background={{ fill: `${color}18` }} dataKey="value" cornerRadius={6} angleAxisId={0}>
    ...
    <div className="container mx-auto px-4 sm:px-6 py-10 max-w-7xl space-y-10">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1.5 text-base">Evaluation metrics and feature importance for the trained Logistic Regression model.</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
        {metricCards.map((m) => <MetricGauge key={m.label} value={m.value} label={m.label} color={m.color} icon={m.icon} />)}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-3 rounded-2xl shadow-sm border-border/60">
          <CardHeader>
            ...
            <CardDescription>Top 3 predictive features by absolute Logistic Regression coefficient.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={featureData} margin={{ top: 10, right: 20, left: -10, bottom: 10 }}>
                  ...
                  <XAxis dataKey="name" tickFormatter={(v) => FEATURE_LABELS[v] ?? v} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} axisLine={false} tickLine={false} />
                  ...
                  <Bar dataKey="impact" radius={[6, 6, 0, 0]} maxBarSize={60}>
                    {featureData.map((_: any, i: number) => <Cell key={i} fill={FEATURE_COLORS[i % FEATURE_COLORS.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
                ...
                <div key={f.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700"
                      style={{ width: `${(f.impact / Math.max(...featureData.map((x: any) => x.impact))) * 100}%`, backgroundColor: FEATURE_COLORS[i] }} />
                  </div>
              ...
              <div className="h-[160px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={distributionData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                    ...
                    <YAxis tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ borderRadius: "10px", border: "1px solid hsl(var(--border))", fontSize: 12 }} cursor={{ fill: "hsl(var(--muted))" }} />
